...
function calculateResults() {
    ...
    for (var i = 0; i < currentQuestions.length; i++) {
        ...
        if (isCorrect === null) reviewItem.style.borderLeftColor = '#0ea5e9';

        var statusIcon = isCorrect === true ? '[Correcto]' : (isCorrect === false ? '[Incorrecto]' : '[Pendiente revision]');
        var expectedHtml = (isCorrect === false) ? '<br>Respuesta correcta: <strong>' + correctAnswerText + '</strong>' : '';
        reviewItem.innerHTML = '<div class="review-q">' + (i + 1) + '. ' + q.question + '</div>' +
            '<div class="review-a">Tu respuesta: <strong>' + answerText + '</strong> ' + statusIcon + expectedHtml + '</div>';
    ...
    var circle = document.getElementById('score-path');
    setTimeout(function() {
    ...
    var message = "Requiere mas entrenamiento en metricas de Salud WFM.";

    if (percentage >= 80) {
        circle.setAttribute('class', 'circle excellent');
        statusClass = 'excellent';
        statusText = 'Aprobado (Test)';
        message = "Excelente en teoria! Respuestas abiertas pendientes de revision.";
    } else if (percentage >= 60) {
        circle.setAttribute('class', 'circle good');
        statusClass = 'good';
        statusText = 'En Revision';
        message = "Buen conocimiento base. Analizar respuestas abiertas para decision final.";
    } else {
